...
async def install_module_endpoint(module_id: str, body: InstallModuleRequest):
    ...
    from src.utils.setup.module_registry import get_manifest, install_module
    try:
        manifest = get_manifest(module_id)
    except FileNotFoundError:
        raise HTTPException(status_code=404, detail=f"Module '{module_id}' not found")
    ...
    install_py = PROJECT_ROOT / "modules" / module_id / "install.py"
    if install_py.exists():
        import importlib.util
        spec = importlib.util.spec_from_file_location(f"modules.{module_id}.install", install_py)
...
async def install_module_from_github(body: GithubInstallRequest):
    ...
    from src.utils.setup.module_registry import install_module
    install_module(module_id)
    invalidate_node_registry()
...
async def uninstall_module_endpoint(module_id: str):
    """Uninstall a module."""
    from src.utils.setup.module_registry import get_manifest, uninstall_module
    try:
        get_manifest(module_id)
    except FileNotFoundError:
        raise HTTPException(status_code=404, detail=f"Module '{module_id}' not found")
